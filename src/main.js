...
const ARVIO_SESSION_KEY="arvioLocalSession_v350";
const ARVIO_PROFILE_KEY="arvioLocalProfile_v350";
...
function readLocalProfile(){
  try{
    const parsed=JSON.parse(localStorage.getItem(ARVIO_PROFILE_KEY)||"null");
    return parsed && typeof parsed==="object" ? parsed : {};
  }catch{return {}}
}
function writeLocalProfile(next){
  try{localStorage.setItem(ARVIO_PROFILE_KEY,JSON.stringify(next||{}))}catch{}
}
function updateLocalProfile(partial){
  const next={...readLocalProfile(),...(partial||{})};
...
function splashToWorkspace(){
  ...
  setTimeout(()=>screens.loading.classList.add("splash-logo-release"),120);
  setTimeout(()=>screens.loading.classList.add("splash-depart","splash-depart-home"),250);
...
// removed without touching localStorage or IndexedDB notes.
if("serviceWorker" in navigator && /^https?:$/.test(location.protocol)){
  window.addEventListener("load",()=>{
    ...
    if("caches" in window){
      caches.keys()
        .then(names=>Promise.allSettled(names.filter(name=>name.startsWith("arvio-")).map(name=>caches.delete(name))))
        .catch(()=>{});
    }
  },{once:true});
}

// Remove the recovery marker from the visible URL after a successful clean load.
try{
  const recoveryUrl=new URL(location.href);
  if(recoveryUrl.searchParams.has("_arvio_refresh")){
    recoveryUrl.searchParams.delete("_arvio_refresh");
...
document.querySelector("#signup-form").addEventListener("submit",e=>{
  ...
  if(!signupEmail){
    setAuthMessage("signup","Enter an email to continue.");
    return;
  }
...
/* ==========================================================
   v3.5.0 — Local-first baseline
   Cloud sync is intentionally removed until a backend is reintroduced.
   IndexedDB remains the source of truth for notes in this build.
   ========================================================== */
...
let libraryStateSaveChain=Promise.resolve();
// Ordered stable IDs belong to the Library snapshot, including hidden Trash pins.
let quickAccessIds=[];
let quickAccessActiveId=null;
let quickAccessOpenRun=0;
    ...
    try{
      ...
      try{
        localStorage.setItem(ARVIO_LIBRARY_STATE_KEY,JSON.stringify({
          ...
          quickAccessIds:snapshot.quickAccessIds
        }));
        return true;
      }catch{return false}
    }
  };
  libraryStateSaveChain=libraryStateSaveChain.then(write,write);
  return libraryStateSaveChain;
}

function persistLibraryState(){
  if(!libraryStateHydrated) return;
  renderQuickAccess();
...
function restoreLegacyPersistedLibraryState(){
  try{
    ...
    quickAccessIds=normalizeQuickAccessIds(saved.quickAccessIds);
...
function normalizeQuickAccessIds(value){
  return Array.isArray(value)
    ? [...new Set(value.filter(id=>typeof id==="string" && id.length>0))]
    : [];
}

function findLibraryNodeById(id,nodes=libraryTree,parentPath=[]){
  if(!id) return null;
  for(let index=0;index<nodes.length;index++){
    const node=nodes[index];
    const path=[...parentPath,node.title];
    if(node.id===id) return {node,path,parentPath,container:nodes,index};
    const child=findLibraryNodeById(id,node.children||[],path);
...
function renderQuickAccess(){
  ...
  if(!libraryStateHydrated) return;
  const byId=new Map(flattenTree(libraryTree).map(entry=>[entry.node.id,entry]));
  const entries=quickAccessIds.map(id=>byId.get(id)).filter(Boolean);
  ...
  }).join("") : '<p class="quick-access-empty">Pin notes from their ••• menu.</p>';
}

function removeQuickAccessSubtree(node){
  const ids=new Set(flattenTree([node]).map(entry=>entry.node.id));
  quickAccessIds=quickAccessIds.filter(id=>!ids.has(id));
}

document.querySelector("#quick-access-list")?.addEventListener("click",async e=>{
  const id=e.target.closest("[data-quick-access-id]")?.dataset.quickAccessId;
  if(!id || !libraryStateHydrated) return;
  const run=++quickAccessOpenRun;
  if(document.body.classList.contains("note-route-active")){
    if(activeLocalNoteKey===`noteid:${id}`) return;
    const cleanup=cleanupEmptyActiveDraft();
    if(!cleanup.removed && !await flushLocalSave()) return;
  }
...
function stampSubtreeUpdatedAt(node,iso){
  node.updatedAt=iso;
  (node.children||[]).forEach(child=>stampSubtreeUpdatedAt(child,iso));
}
...
function removeLibraryPath(path,{recordTrash=false,noteId=null}={}){
  ...
  if(recordTrash){
    ...
    const trashItem={
      ...
      parentId:found.parentPath.length ? findLibraryNode(found.parentPath)?.node?.id : null,
...
function restoreLibraryTrashItem(id){
  ...
  const parent=item.parentId ? findLibraryNodeById(item.parentId) : null;
  const parentPath=parent?.path || item.parentPath;
  const container=item.parentId
    ? (parent ? (parent.node.children ??= []) : null)
    : getLibraryContainerForParent(parentPath);
  if(!container) return {ok:false,missingParent:true};

  // A replacement note may now occupy the old name. Keep both identities reachable.
  item.node.title=uniqueLibraryTitle(container,item.node.title);
  item.parentPath=parentPath;
  item.path=[...parentPath,item.node.title];
  repairLibraryCreationMetadata([item.node],parentPath);
...
function applyPersistedLibraryTrash(){
  ...
  [...libraryTrash].reverse().forEach(item=>{
    // Never remove a newly-created live note that happens to reuse a trashed path.
    removeLibraryPath(item.path,{recordTrash:false,noteId:item.node?.id || null});
  });
}
...
async function hydrateLibraryStateFromIndexedDB(){
  ...
  try{
    ...
    if(saved?.tree && Array.isArray(saved.tree)){
      ...
      quickAccessIds=normalizeQuickAccessIds(saved.quickAccessIds);
      repairedIds=ensureLibraryNodeIds();
      libraryTrash.forEach(item=>{
        if(ensureLibraryNodeIds([item.node])) repairedIds=true;
      });
      try{
        localStorage.removeItem(ARVIO_LIBRARY_STATE_KEY);
        localStorage.removeItem(ARVIO_LIBRARY_TRASH_KEY);
    ...
    if(repairedCreationMetadata || repairedIds) await persistLibraryStateNow();
...
function cleanupEmptyActiveDraft(){
  ...
  removeQuickAccessSubtree(found.node);
  removeLibraryPath(removedPath,{recordTrash:false,noteId:found.node.id});
...
function markNoteOpened(path,noteId=null){
  const found=noteId ? findLibraryNodeById(noteId) : findLibraryNode(path);
  if(!found) return;
...
function openLibraryItemActions(pathKey,trigger){
  ...
  overlay.innerHTML=`
    <div class="library-item-action-sheet" role="dialog" aria-modal="true" aria-label="${escapeHtml(found.node.title)} actions">
      ...
      <div class="library-item-action-list">
        <button class="library-item-action" type="button" data-library-action="quick-access" aria-pressed="${quickAccessIds.includes(found.node.id)}" ${libraryStateHydrated?"":"disabled"}>
  ...
  const positionSheet=()=>{
    ...
    // offset dimensions exclude the entry transform. Re-measure wrapped content.
    sheet.style.left=`${Math.max(12,Math.min(window.innerWidth-sheet.offsetWidth-12,(rect?.right || window.innerWidth/2)-sheet.offsetWidth))}px`;
    sheet.style.top=`${Math.max(12,Math.min(window.innerHeight-sheet.offsetHeight-12,(rect?.bottom || 12)+8))}px`;
  };
  positionSheet();
  const positionController=new AbortController();
  window.addEventListener("resize",positionSheet,{signal:positionController.signal});
  window.visualViewport?.addEventListener("resize",positionSheet,{signal:positionController.signal});
  window.visualViewport?.addEventListener("scroll",positionSheet,{signal:positionController.signal});
  const sheetResizeObserver=new ResizeObserver(positionSheet);
  ...
  overlay.addEventListener("click",async e=>{
    ...
    if(action==="quick-access"){
      ...
      if(button.disabled || !libraryStateHydrated || !findLibraryNodeById(found.node.id)) return;
      button.disabled=true;
      const before=[...quickAccessIds];
      const wasPinned=before.includes(found.node.id);
      quickAccessIds=wasPinned ? before.filter(id=>id!==found.node.id) : [found.node.id,...before];
      renderQuickAccess();
      const saved=await persistLibraryStateNow();
      if(!saved){
        quickAccessIds=before;
        renderQuickAccess();
        button.querySelector("small").textContent="Couldn’t save. Please try again.";
      }else{
        button.setAttribute("aria-pressed",String(!wasPinned));
        button.querySelector("strong").textContent=wasPinned?"Add to Quick Access":"Remove from Quick Access";
        button.querySelector("small").textContent=wasPinned?"Removed from Quick Access.":"Pinned to Quick Access.";
      }
...
function permanentlyDeleteLibraryTrashItems(ids){
  ...
  deleting.forEach(item=>{
    removeQuickAccessSubtree(item.node);
...
function findActiveLibraryNote(){
  return activeLocalNoteKey.startsWith("noteid:")
    ? findLibraryNodeById(activeLocalNoteKey.slice(7))
    : findLibraryNode(activeNotePath);
}
...
async function restoreLocalNoteIfPresent(key,legacyKey=null){
  const openingRevision=noteRevision;
  const local=await loadNoteLocally(key,legacyKey);
  // A slow read for the previous note must never replace the current editor.
  if(key!==activeLocalNoteKey || openingRevision!==noteRevision || noteDirty) return;
  if(!local){
    noteDirty=false;
    setNoteSaveStatus("saved","Saved");
    return;
